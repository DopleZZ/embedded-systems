...
- `soil/data` — телеметрия устройства (JSON), содержит `deviceUid` и блок `measurements`.
- `soil/cmd` — команды управления устройствами.

=== Команды

- `get_info` — запросить немедленную публикацию телеметрии (широковещательно, все устройства могут ответить).
- `water;<deviceUid>;<durationMs>` — включить помпу полива на заданное время (команда применяется только устройством с совпадающим `deviceUid`).
- `water_stop;<deviceUid>` — немедленно выключить помпу (опционально).
